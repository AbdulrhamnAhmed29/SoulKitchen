                        ...
                        {isToken ? (
                            <>
                                <Link to="/cart" className="relative p-2 text-white hover:text-white/70 transition-colors">
                                    ...
                                    <span className="absolute top-0 right-0 flex h-4 w-4 items-center justify-center rounded-full bg-zinc-800 text-[8px] font-bold shadow-lg border border-white/10">0</span>
                                </Link>
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        ...
                        {/* Premium User Profile Section (Total Dark Style) */}
                        {isToken && (
                            ...
                            {navLinks.map((link, index) => (
                                ...
                                >
                                    <Link
                                        ...
                                        className="text-[11px] tracking-[0.4em] text-white/60 hover:text-white hover:translate-x-2 transition-all duration-300 block uppercase"
                                    >
                                        {link.name}
                                    </Link>
                                </motion.div>
                            ...
                            {isToken ? (
                                <div className="flex flex-col gap-6">
                                    <Link to="/cart" onClick={() => setIsOpen(false)} className="flex items-center gap-4 text-white/80 hover:text-white transition-colors group">
                                        ...
                                        <span className="text-[10px] tracking-[0.3em] uppercase">My Cart (0)</span>
                                    </Link>
                                ...
                                <div className="flex flex-col gap-4 mt-2">
                                    <Link to="/signin" onClick={() => setIsOpen(false)} className="w-full py-4 text-center border border-white/10 text-[10px] tracking-[0.5em] text-white/80 hover:bg-white hover:text-black transition-all font-bold">
                                        LOGIN
                                    </Link>
                                    <Link to="/signup" onClick={() => setIsOpen(false)} className="w-full py-4 text-center bg-white text-[10px] tracking-[0.5em] text-black hover:bg-white/90 transition-all font-bold shadow-lg shadow-white/5">
                                        REGISTER
                                    </Link>
                                </div>
                            )}
                        ...
                        <div className="mt-auto pt-10 border-t border-white/5">
                            <p className="text-[7px] tracking-[0.6em] text-white/10 uppercase text-center font-bold">Soul Kitchen • Gastronomy Experience</p>
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>

            {/* Overlay (Total Dark Style) */}
            <AnimatePresence>
                {isOpen && (
                    <motion.div 
                        initial={{ opacity: 0 }}
                        ...
                        exit={{ opacity: 0 }}
                        onClick={() => setIsOpen(false)} 
                        className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[105] lg:hidden"
                    />
                )}
            </AnimatePresence>
        </nav>
